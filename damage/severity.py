...
def _otsu(values, bins=64):
    """Threshold that best separates a 1-D sample into two groups.
...
def depth_score(m):
    ...
    # Neutral paint: only lightness is left, and it has to carry the whole
    # score. A scratch to primer on a black car goes light; on a white car it
    # goes dark. Absolute value, because the direction depends on the paint
    # and the depth does not.
    return min(1.0, abs(m["dL"]) / 45.0), "lightness"
...
def _selftest():
    ...
    check("otsu survives a constant sample", _otsu(np.full(50, 7.0)) == 7.0)
    check("otsu survives an empty sample", _otsu([]) == 0.0)
