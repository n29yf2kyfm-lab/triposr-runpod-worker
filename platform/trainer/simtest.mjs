...
const p = await b.newPage({ viewport: { width: 1280, height: 860 }, ignoreHTTPSErrors: true });
...
async function solveOrder(maxWait = 12000) {
  for (let guard = 0; guard < 60; guard++) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (after === before) { // accepted: wait for its animation to finish
        const t0 = Date.now(); while (await p.evaluate(() => __sim.api.busy) && Date.now() - t0 < maxWait) await sleep(100);
...
await job('misfire', async () => {
  ...
  await click('Cylinder ' + s.st.cyl); await p.screenshot({ path: out + '/misfire1.png' }); await next();
  s = await st();
  ...
  await p.screenshot({ path: out + '/misfire2.png' }); await next();
...
await job('starter', async () => {
  ...
  await click('Starter faulty'); await p.screenshot({ path: out + '/starter2.png' }); await next();
  await solveOrder(); await p.screenshot({ path: out + '/starter3.png' }); await next();
  await solveOrder(); await next();
});
await job('alternator', async () => {
  await click('Start engine'); await p.locator('#sim-panel .chip').first().click(); await sleep(800);
  await p.screenshot({ path: out + '/alt1.png' });
  ...
  await click(map[s.st.fault]); await p.screenshot({ path: out + '/alt2.png' }); await next();
  await solveOrder(); await next();
...
await job('brakes', async () => {
  ...
  await solveOrder(); await p.screenshot({ path: out + '/brakes3.png' }); await next();
  await solveOrder(); await next();
  await solveOrder(); await next();
  ...
  await p.screenshot({ path: out + '/brakes6.png' }); await next();
  await click('Pump the pedal'); await next();
  console.log('brakes worn disc', s.st.worn);
});
await job('airbag', async () => {
  await solveOrder(15000); await next();
  await solveOrder(); await p.screenshot({ path: out + '/airbag2.png' }); await next();
  await solveOrder(); await next();
});
await p.screenshot({ path: out + '/report.png' });
await p.goto(url + '#sim'); await sleep(800); await p.screenshot({ path: out + '/home2.png', fullPage: true });
const m = await b.newPage({ viewport: { width: 390, height: 844 }, deviceScaleFactor: 2, colorScheme: 'dark', ignoreHTTPSErrors: true });
await m.goto(url + '#sim-brakes'); await m.waitForTimeout(2500); await m.screenshot({ path: out + '/mobile.png', fullPage: true });
