...
| `o-voxel/o_voxel/postprocess.py` | `to_glb` auto-switches the material to `alphaMode='BLEND'` when the baked alpha texture contains translucency (opt out with `GLB_ALPHA_MODE=opaque`) | upstream hardcodes `OPAQUE`, so the real per-texel glass opacity the model generates was ignored and windows rendered solid |
...
| `trellis2/pipelines/trellis2_image_to_3d.py` | **Added** `get_cond_multi()` and `run_multi_image()` — multi-VIEW image-to-3D. `run()` is untouched. | one object from several photos (front/side/rear), e.g. a damage-capture set, produces a far better model than a single view whose unseen sides are hallucinated. The feature extractor already returns `(V, N, D)` for a list of images; `get_cond_multi` concatenates the per-view tokens along the sequence axis to `(1, V*N, D)` so one sample attends to all views (the standard TRELLIS multi-image method — attention is length-flexible, no retraining). **NEEDS GPU VALIDATION:** token-concatenation multi-view is unverified on this specific checkpoint; confirm quality on a pod before relying on it. Single-view callers are unaffected (a 1-image list reproduces `run()`'s conditioning exactly). |
